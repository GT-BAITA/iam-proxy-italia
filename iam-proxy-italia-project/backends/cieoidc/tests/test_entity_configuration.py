...
def handler(minimal_config):
    with patch(
        "backends.cieoidc.utils.validators.validate_private_jwks"
    ) as mock_val_jwks, patch(
        "backends.cieoidc.utils.validators.validate_entity_metadata"
    ) as mock_val_meta:
...
def test_metadata_property(handler):
    with patch(
        "backends.cieoidc.utils.helpers.jwks.public_jwk_from_private_jwk"
    ) as mock_pub:
        ...
        assert (
            meta["openid_relying_party"]["jwks"]["keys"][0]["kid"]
            == "YhuIJU6o15EUCyqA0LHEqJd-xVPJgoyW5wZ1o4padWs"
        )
...
@patch(
    "backends.cieoidc.endpoints.entity_configuration.EntityConfigHandler.get_entity_configuration"
)
...
@patch(
    "backends.cieoidc.endpoints.entity_configuration.EntityConfigHandler.get_openid_jwks"
)
